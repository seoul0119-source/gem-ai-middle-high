...
await fs.mkdir(dist+'/media',{recursive:true});await fs.copyFile('mission/media/join-clip.mjs',dist+'/media/join-clip.mjs');
...
let sw=await fs.readFile(dist+'/sw.js','utf8');sw=sw.replace('gem-g2-shell-diagnostics-v3','gem-group-classroom-v4').replace("'./lesson.mjs'","'./lesson.mjs','./group-classroom.mjs','./classroom-catalog.mjs','./group-style.css','./lesson-media.mjs','./media/join-clip.mjs'");await fs.writeFile(dist+'/sw.js',sw);
