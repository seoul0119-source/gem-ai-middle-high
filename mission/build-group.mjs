...
for(const name of ['group-classroom.mjs','classroom-catalog.mjs','group-style.css','lesson-media.mjs'])await fs.copyFile('mission/'+name,dist+'/'+name);
await fs.mkdir(dist+'/media',{recursive:true});for(const file of ['join-clip.mjs','count-on-clip.mjs'])await fs.copyFile('mission/media/'+file,dist+'/media/'+file);
...
// Emit only a final recognition result, once, after the microphone has closed.
...
app="import {spokenMath} from './lesson-media.mjs';\n"+app.replace('message.match(/[^.!?]+[.!?]?/g)', 'spokenMath(message,state.lang).match(/[^.!?]+[.!?]?/g)');
...
let sw=await fs.readFile(dist+'/sw.js','utf8');sw=sw.replace('gem-g2-shell-diagnostics-v3','gem-group-classroom-v6').replace("'./lesson.mjs'","'./lesson.mjs','./group-classroom.mjs','./classroom-catalog.mjs','./group-style.css','./lesson-media.mjs','./media/join-clip.mjs','./media/count-on-clip.mjs'");await fs.writeFile(dist+'/sw.js',sw);
