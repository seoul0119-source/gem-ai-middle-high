...
// Runs after the audio-v2 transformation. Limit this change to the diagnostic UI.
let app=await fs.readFile('mission/app.mjs','utf8');
if(!app.includes('// GEM_DIAGNOSTICS_DIALOG_V3')){
 const anchor="render();if(restored)notice(text('saved'));";
 if(!app.includes('// GEM_MEDIA_FIX_V2')||app.split(anchor).length!==2)throw Error('Expected audio-v2 source template before diagnostics patch');
 app="// GEM_DIAGNOSTICS_DIALOG_V3\nimport {installDiagnosticsDialog} from './diagnostics.mjs';\n"+app.replace(anchor,"render();installDiagnosticsDialog({refresh:updateVoices,stop:()=>{stopSpeech();stopMic();stopMicCheck();if(state.started&&!state.paused&&!state.ended)pause();}});if(restored)notice(text('saved'));");
 await fs.writeFile('mission/app.mjs',app);
 let sw=await fs.readFile('mission/sw.js','utf8');
 if(!sw.includes('gem-g2-shell-audio-v2')||!sw.includes("'./lesson.mjs'"))throw Error('Unexpected service worker before diagnostics patch');
 sw=sw.replace('gem-g2-shell-audio-v2','gem-g2-shell-diagnostics-v3').replace("'./lesson.mjs'","'./lesson.mjs','./diagnostics.mjs'");
 await fs.writeFile('mission/sw.js',sw);
 let html=await fs.readFile('mission/index.html','utf8');
 html=html.replace('운영자 음성 수정 2차 ·','운영자 진단창 수정 3차 ·');await fs.writeFile('mission/index.html',html);
}
execFileSync(process.execPath,['mission/tests.mjs'],{stdio:'inherit'});for(const f of ['app.mjs','avatar.mjs','sw.js','diagnostics.mjs'])execFileSync(process.execPath,['--check','mission/'+f],{stdio:'inherit'});
const dist='mission-dist';await fs.mkdir(dist+'/assets',{recursive:true});for(const f of ['index.html','style.css','app.mjs','lesson.mjs','sw.js','diagnostics.mjs'])await fs.copyFile('mission/'+f,dist+'/'+f);
...
await import('./diagnostics-browser-tests.mjs');
