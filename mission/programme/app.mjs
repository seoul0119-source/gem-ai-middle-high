...
function stopSpeech(){speechEpoch++;clearTimeout(voiceTimer);audio?.cancelVoiceWait();globalThis.speechSynthesis?.cancel();talk(false);}
function stop(){stopSpeech();audio?.stopMic();cancelRequest();dialogue=false;followup=0;}
...
function say(message){
 stopSpeech();if(!message||document.hidden)return;$('caption').textContent=message;const v=audio.voices().find(v=>v.voiceURI===$('voices').value)||audio.voices()[0];
 if(!v||!globalThis.SpeechSynthesisUtterance){audio.noteTTS(audio.missingVoiceCode());audio.waitForVoice(()=>say(message));return;}
 ...
 u.onstart=()=>{if(token===speechEpoch)audio.noteTTS('speaking');};u.onend=()=>{if(token===speechEpoch){clearTimeout(voiceTimer);talk(false);audio.noteTTS('completed');}};u.onerror=e=>{if(token===speechEpoch){stopSpeech();audio.noteTTS(e.error||'synthesis-failed');}};
 try{speechSynthesis.speak(u);}catch{stopSpeech();audio.noteTTS('synthesis-failed');}
}
...
function updateControls(){
 ...
 $('mic').disabled=busy||!step();$('answer').disabled=busy;$('answer-form').querySelector('button').disabled=busy;
...
function render(){
 ...
 $('answer').placeholder=T('ask');$('answer').setAttribute('aria-label',T('ask'));updateControls();audio.refresh();audio.render();save();dispatchEvent(new Event('gem-display-resize'));
}
...
function adopt(unitId,lesson,id){stop();state={...state,unitId,lesson,sessionId:id,index:0,elapsed:0,paused:true,started:false,ended:false,approved:false,revealed:{},choices:{},draft:'',audioInput:null};$('answer').value='';lastReply=null;history=[];statusKey='readyLesson';cacheLesson();render();}
...
async function submit(event,source='text'){
 event?.preventDefault();const q=$('answer').value.trim();if(!q||busy||!step())return;
 const lang=state.lang,which=key();audio.prepareSubmission(q,source);beginDialogue();const index=step().kind==='question'&&!revealed()?choiceFor(q,pack().options):-1;
...
$('next').onclick=()=>move(1);$('previous').onclick=()=>move(-1);$('repeat').onclick=()=>say(lastReply?.text||(revealed()?pack().explanation:pack().narration));$('answer-form').onsubmit=submit;$('mic').onclick=()=>audio.listen();
