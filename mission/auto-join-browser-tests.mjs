...
try{
 ...
 assert.match(await page.locator('#auto-join-visual [data-formula]').textContent(),/= \?/);assert.equal(await page.evaluate(()=>GEM_AUTO_MEDIA.mediaCount),2);
 ...
 assert.match(await page.locator('#auto-join-visual [data-formula]').textContent(),new RegExp('= '+total+'$'));assert.equal(await page.locator('#auto-join-visual [data-counter]').count(),total);
