...
## Second problem: count on, eight plus two

`gem-g2-math-count-on-8-2` is the second incremental package, with a precise number-line diagram, original eight-second muted video, key fact and worked six-plus-two example. EN/FR narration and images share one content ID. Its pair activity and prepared answers explain why the starting eight is not counted as an extra step. The summary question window uses existing controls. The package registry allows the remaining Grade 2 questions to be added independently. Remaining questions still use the earlier pilot material and have not been upgraded to full media packages.
