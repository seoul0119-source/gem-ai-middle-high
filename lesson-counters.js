(function(root){
 ...
 function apply(state,event){
  ...
  if(event.completed||event.event==='correct')q.completed=true;
 ...
 function restore(messages){const state=create();for(const m of messages||[]){if(m.role==='assistant'){if(m.progress)apply(state,m.progress);else state.uncertain=true;}}return state;}
 root.GemLessonCounters={create,apply,values,restore};
})(globalThis);
