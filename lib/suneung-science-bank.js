...
function questionByNumber(number) {
  return CLOSED_SUNEUNG_SCIENCE_QUESTIONS[number - 1] || null;
}
...
export function isValidClosedScienceRecord(record) {
...
function isApprovedSummary(value, finalOutcome = "") {
  ...
  const bankTopicIndexes = topics.map((topic) =>
    CLOSED_SUNEUNG_SCIENCE_QUESTIONS.findIndex((question) => question.topic === topic)
  );
...
export function projectClosedSuneungScienceSpeechText(value) {
  return String(value || "")
...
export function isApprovedClosedSuneungScienceResponse(value) {
...
export function isApprovedClosedSuneungScienceSpeechText(value) {
  const text = String(value || "");
  if (!text || text.length > 10000 || containsExcludedSuneungScienceContent(text)) return false;
...
export function handleClosedSuneungScienceLesson({
  courseId,
  messages,
  learningProfile,
  blockedInput = false
