...
import { science2027Questions } from "./suneung-science-2027.js";
import { resolveScienceContentAnswer } from "./science-answer.js";
import {
  resolveScienceHelp, relatedScienceConcepts, renderScienceConceptHelp,
  readScienceConceptHelp, isApprovedScienceConceptSpeech, namedScienceConcepts
} from "./suneung-science-help.js";
import {
...
export function createScienceLessonEngine(seed = "", courseId = GUARDED_SUNEUNG_SCIENCE_COURSE_ID) {
const engineCourseId = courseId;
const lessonQuestions = science2027Questions(courseId, seed)
  || scienceVariants(CLOSED_SUNEUNG_SCIENCE_QUESTIONS, seed);
...
function normalizeChoicePhrase(value) {
  return String(value || "").normalize("NFKC").trim()
    .replace(/[.!?。]+$/g, "")
    .replace(/^(?:정답|답)(?:은|는)?\s*/, "")
    .replace(/(?:가|이)\s*정답(?:입니다|이에요|예요)?$/, "")
    .trim();
}

function parseChoice(value) {
  const normalized = normalizeChoicePhrase(value).replace(/^(?:알파벳|영어)\s*/i, "").toUpperCase();
...
function parseKoreanSpokenChoice(value) {
  const phrase = normalizeChoicePhrase(value);
  // Preserve the 이 in an explicit alphabet answer before stripping polite
  // endings: “알파벳 이요” contains the choice itself as part of “이요”.
  if (/^(?:알파벳|영어)\s*이(?:\s*번)?(?:\s*(?:입니다|에요|예요|이에요|이요|요))?$/.test(phrase)) return "E";
  ...
  const spokenChoices = {
    ...
    "알파벳이":"E", "알파벳이번":"E", "영어이":"E"
  };
  return spokenChoices[compact] || "";
}

function isAmbiguousVoiceChoice(value) {
  // Korean E (이/이 번) and the number 2 sound alike. Never guess a key
  // or consume a learner's attempt when ASR outputs either interpretation.
  return /^(?:2|이)(?:\s*번)?(?:\s*(?:입니다|이에요|예요|이요|요))?$/.test(normalizeChoicePhrase(value));
}

function renderVoiceClarification(question, attempt) {
  return `문제 ${question.number}/10 · 음성 답안 확인 · 도전 ${attempt}/3\n알파벳 E와 숫자 2의 발음이 비슷해서 답안을 확인하겠습니다. 아직 채점하지 않았습니다. E라면 ‘알파벳 이’ 또는 ‘5번’, B라면 ‘비’ 또는 ‘두 번째’라고 말해 주세요. 화면의 E 또는 B 버튼을 눌러도 됩니다.\n\n답: (________)`;
}

function renderContentClarification(question, attempt) {
  return `문제 ${question.number}/10 · 답안 확인 · 도전 ${attempt}/3\n말씀하신 내용이 여러 보기와 겹쳐 아직 채점하지 않았습니다. 답에 단위를 함께 적거나 A~E 중 하나를 선택해 주세요.\n\n답: (________)`;
}

const SKIP_LEAD = "현재 문제는 건너뛰기로 기록했습니다. 정답·오답 점수에는 포함하지 않습니다. 새로운 문제를 풀어 보세요.";

function submittedChoice(value) {
  return parseChoice(value) || parseKoreanSpokenChoice(value);
}

function explicitLessonCommand(value) {
  const text = String(value || "").normalize("NFKC").trim();
  if (/^(?:새(?:로운)?\s*문제|다른\s*문제|다음\s*문제|건너뛰기|건너뛰어)(?:\s*(?:를|로))?(?:\s*(?:주세요|내\s*주세요|부탁해요|넘어가\s*주세요))?[.!?。]*$/.test(text)) return "skip";

  const compact = text.replace(/\s+/g, "").replace(/[.!?。]+$/g, "");
  if (/^(?:(?:안녕하세요|선생님)[,.!。]?)?(?:(?:수업|학습|문제)(?:을|를)?)?(?:다시)?(?:시작|계속)(?:해)?(?:주세요|줘|줘요|부탁해요|부탁드립니다|할게요|하겠습니다)?$/.test(compact)
    || /^(?:(?:수업|학습)(?:을)?)?이어서(?:해)?(?:주세요|줘|줘요)?$/.test(compact)) return "start";

  if (/^(?:다시|한번더)(?:문제|질문|보기)?$/.test(compact)
    || /^(?:(?:이|현재|지금)?(?:문제|질문|보기)(?:를|을)?)?(?:(?:다시|한번더)(?:(?:문제|질문|보기)(?:를|을)?)?)?(?:읽어|보여)(?:주세요|줘|줘요)?$/.test(compact)) return "repeat";
  return "";
}
...
function requestedTerm(text, question) {
  if (!/(?:무엇|뭐|뭔|의미|뜻|설명|알려|왜|어떤)/.test(text)) return "";
  const match = text.match(/(?:^|\s)([A-E씨디비])(?:\s*(?:번|보기|선택지))?(?:가|이|는|은|의|\s|$)/i);
  ...
  return Object.keys(glossary).find(term => ((term === "줄"
    ? /(?:^|\s)줄(?:[?!]|[은이의을란에]|$|\s*(?:단위|뜻|의미|설명))/.test(text) : text.includes(term)) || referenced.includes(term))
    && (question.stem.includes(term) || question.choices.some(choice => choice.includes(term)) || term === "SI")) || "";
}

function renderQuestionHelp(question, attempt) {
  return `문제 ${question.number}/10 · 풀이 도움 · 도전 ${attempt}/3\n이 질문은 답안으로 채점하지 않습니다. 보기의 정답 여부를 결정하기 전에 풀이 방법을 함께 살펴볼게요.\n첫째, ${question.hints[0]}\n둘째, ${question.hints[1]}\n궁금한 용어나 개념을 말하면 더 설명해 드릴게요.\n\n답: (________)`;
}

function renderHelpClarification(question, attempt) {
  const topics = relatedScienceConcepts(question).map(concept => concept.title).join(", ");
  return `문제 ${question.number}/10 · 질문 확인 · 도전 ${attempt}/3\n어떤 부분이 궁금한지 조금 더 구체적으로 말씀해 주세요. ${topics || question.topic} 등의 개념을 설명할 수 있습니다. ‘화학 결합이 무엇인가요?’, ‘이 문제 풀이를 도와주세요’처럼 질문해 주세요. 질문은 답안으로 채점하지 않습니다.\n\n답: (________)`;
}
...
function validatedCompletedRecords(profile) {
  ...
  for (const candidate of candidates) {
    ...
    if (!['correct', 'incorrect', 'skipped'].includes(outcome)) continue;
    if ((outcome === "skipped") !== (attempts === 0)) continue;
...
// Callers may replace an authenticated conversational reply with this exact
// server-rendered text before recovering state. Never recover attempts from an
// arbitrary assistant header or from client-supplied question metadata.
function canonicalQuestionForState(number, attempt = 1) {
  if (!Number.isInteger(number) || !Number.isInteger(attempt) || attempt < 1 || attempt > 3) return "";
  const question = questionByNumber(number);
  return question ? renderQuestion(question, attempt) : "";
}

function getConversationTurn({ messages, learningProfile, inputMode = "text" } = {}) {
  const safeMessages = Array.isArray(messages) ? messages : [];
  const completed = validatedCompletedRecords(learningProfile);
  const question = questionByNumber(completed.length + 1);
  if (!question) return { kind:"control", action:"complete", question:null, attempt:0, canonicalQuestion:"" };

  const attempt = latestDisplayedAttempt(safeMessages, question.number) || 1;
  const state = { question, attempt, canonicalQuestion:renderQuestion(question, attempt) };
  const latestUser = safeMessages[safeMessages.length - 1];
  if (latestUser?.role !== "user") return { kind:"control", action:"show", ...state };

  const submitted = String(latestUser.content || "");
  if (inputMode === "voice" && isAmbiguousVoiceChoice(submitted)) {
    return { kind:"control", action:"ambiguous-choice", ...state };
  }
  if (submittedChoice(submitted)) return { kind:"control", action:"answer", ...state };
  const contentAnswer = resolveScienceContentAnswer(submitted, question);
  if (contentAnswer) return { kind:"control", action:contentAnswer.kind === "answer" ? "answer" : "ambiguous-content", ...state };
  const action = explicitLessonCommand(submitted);
  if (action) return { kind:"control", action, ...state };

  // Natural conversation is the default, including unfamiliar concepts,
  // follow-up questions and hints. Only complete, explicit control utterances
  // can reach the deterministic grading path.
  return { kind:"conversation", ...state };
}
...
function isValidClosedScienceRecord(record) {
  ...
  if (!question || !Number.isInteger(attempts) || attempts < 0 || attempts > 3) return false;
  if (record.stage !== question.stage || record.topic !== question.topic || record.scope !== "direct") return false;
  if (!['correct', 'incorrect', 'skipped'].includes(record.outcome)) return false;
  if ((record.outcome === "skipped") !== (attempts === 0)) return false;
...
function renderHint(question, hintNumber, attempt) {
  if (hintNumber > 2) {
    return `문제 ${question.number}/10 · 힌트 2/2 · 도전 ${attempt}/3\n힌트는 두 단계까지 제공됩니다. 두 힌트를 함께 정리해 드릴게요.\n첫째, ${question.hints[0]}\n둘째, ${question.hints[1]}\n아직 어렵다면 ‘쉽게 설명해 주세요’ 또는 궁금한 개념을 말해 주세요. 도전 횟수는 그대로입니다.\n\n답: (________)`;
  }
...
function renderSummary(records) {
  ...
  return [
    "이번 통합과학 수업을 마쳤습니다.",
    `정답 ${correct}문제 · 오답 ${incorrect}문제${skipped ? ` · 건너뛰기 ${skipped}문제` : ""}`,
...
function isApprovedSummary(value, finalOutcome = "") {
  ...
  const totals = lines[1].match(/^정답 (10|[0-9])문제 · 오답 (10|[0-9])문제(?: · 건너뛰기 (10|[1-9])문제)?$/);
  if (!totals || Number(totals[1]) + Number(totals[2]) + Number(totals[3] || 0) !== 10) return false;
  if (finalOutcome === "correct" && Number(totals[1]) < 1) return false;
  if (finalOutcome === "skipped" && Number(totals[3] || 0) < 1) return false;
...
function buildApprovedStaticResponses() {
  ...
  for (const question of lessonQuestions) {
    for (let attempt = 1; attempt <= 3; attempt += 1) {
      ...
      approved.add(renderVoiceClarification(question, attempt));
      approved.add(renderContentClarification(question, attempt));
      ...
      // Retain recognition of a hint displayed before this deployment.
      approved.add(`문제 ${question.number}/10 · 힌트 2/2 · 도전 ${attempt}/3\n힌트는 두 단계까지 제공됩니다. 이제 A~E 또는 1~5 중 하나로 답해 주세요.\n\n답: (________)`);
      approved.add(renderQuestionHelp(question, attempt));
      approved.add(renderHelpClarification(question, attempt));
    ...
    if (question.number < 10) {
      ...
      approved.add(renderQuestion(nextQuestion, 1, SKIP_LEAD));
...
function isApprovedClosedSuneungScienceResponse(value) {
  ...
  if (readScienceConceptHelp(text)) return true;
  ...
  if (lead === SKIP_LEAD) return isApprovedSummary(summary, "skipped");
...
function isApprovedClosedSuneungScienceSpeechText(value) {
  ...
  return APPROVED_STATIC_SPEECH_RESPONSES.has(text)
    || isApprovedScienceConceptSpeech(text)
...
function handleClosedSuneungScienceLesson({
  ...
  if (!isGuardedSuneungScienceCourse(courseId) || courseId !== engineCourseId) return null;
  const safeMessages = Array.isArray(messages) ? messages : [];
  const completed = validatedCompletedRecords(learningProfile);
  ...
  const submitted = String(latestUser?.content || "");
  const command = explicitLessonCommand(submitted);
  if (command === "start" || command === "repeat") {
    return { text:renderQuestion(question, displayedAttempt) };
  }
  if (command === "skip") {
    const record = createRecord(question, "skipped", 0);
    return question.number === 10
      ? { text:`${SKIP_LEAD}\n\n${renderSummary([...completed, record])}`, record }
      : { text:renderQuestion(questionByNumber(question.number + 1), 1, SKIP_LEAD), record };
  }
  if (inputMode === "voice" && isAmbiguousVoiceChoice(submitted)) {
    return { text:renderVoiceClarification(question, displayedAttempt) };
  }
  // Complete choice values are answers too: a learner may say "6.4 cm"
  // instead of its shuffled A–E label. Resolve them before any help router.
  const contentAnswer = submittedChoice(submitted) ? null : resolveScienceContentAnswer(submitted, question);
  if (contentAnswer?.kind === "clarify") {
    return { text:renderContentClarification(question, displayedAttempt) };
  }
  if (contentAnswer?.kind === "answer") {
    return gradeScienceAnswer(question, displayedAttempt, completed, safeMessages, contentAnswer.correct);
  }
  if (!/(?:설명|뜻|의미|왜)/.test(submitted)
    && /(?:다시|한번\s*더|한 번\s*더).*(?:읽|문제|질문)|문제.*(?:읽어|보여)/.test(submitted)) {
    return { text:renderQuestion(question, displayedAttempt) };
  }
  const term = requestedTerm(submitted, question);
  if (term) {
    const concepts = namedScienceConcepts(term, question).slice(0, 1);
    if (concepts.length) {
      const mode = /예시|예를/.test(submitted) ? "example" : /쉽게|풀어서|자세히|차근차근/.test(submitted) ? "simple" : "explain";
      return { text:renderScienceConceptHelp(question, displayedAttempt, concepts, mode) };
    }
    return { text:supportText(question, term, displayedAttempt) };
  }
  if (!/힌트/.test(submitted) || /힌트(?:말고|가아니라|대신)/.test(submitted.replace(/\s+/g, ""))) {
    // A glossary explanation already on screen before this release remains
    // usable as context for “give me an example” after deployment.
    const helpMessages = safeMessages.map(message => {
      if (message.role !== "assistant") return message;
      for (const oldTerm of Object.keys(glossary)) {
        for (let attempt = 1; attempt <= 3; attempt += 1) {
          if (message.content !== supportText(question, oldTerm, attempt)) continue;
          const concepts = namedScienceConcepts(oldTerm, question).slice(0, 1);
          if (concepts.length) return { ...message, content:renderScienceConceptHelp(question, attempt, concepts) };
        }
      }
      return message;
    });
    const help = resolveScienceHelp(submitted, question, helpMessages);
    if (help?.kind === "concept") return { text:renderScienceConceptHelp(question, displayedAttempt, help.concepts, help.mode) };
    if (help?.kind === "question") return { text:renderQuestionHelp(question, displayedAttempt) };
    if (help?.kind === "clarify" && !isHintRequest(submitted)) return { text:renderHelpClarification(question, displayedAttempt) };
  }
  if (isHintRequest(submitted)) {
    return {
      text:renderHint(
        question,
  ...
  return gradeScienceAnswer(question, displayedAttempt, completed, safeMessages, choice === question.answer);
}

function gradeScienceAnswer(question, attempt, completed, safeMessages, correct) {
  if (correct) {
...
return { questions:lessonQuestions, isValidClosedScienceRecord, projectClosedSuneungScienceSpeechText,
  ...
  getConversationTurn, canonicalQuestionForState };
}
...
export const { isValidClosedScienceRecord, projectClosedSuneungScienceSpeechText,
  ...
  handleClosedSuneungScienceLesson, getConversationTurn, canonicalQuestionForState } = createScienceLessonEngine();
