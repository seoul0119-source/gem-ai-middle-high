...
import {isBlockedStudentId,studentRegistration} from './student-session.js';
...
export function issueClassroomPass(student, classroom) {
 ...
 const payload = {purpose:'classroom-entry', ...studentRegistration(student.id), id:student.id, aud:CLASSROOMS[classroom], jti:crypto.randomUUID(), authExp:student.exp, exp:Math.min(now+120,student.exp)};
...
export function exchangeRecordPass(ticket,audience){
 ...
 const encoded=Buffer.from(JSON.stringify({purpose:'record-session',...studentRegistration(p.id),id:p.id,aud:p.aud,exp:p.authExp})).toString('base64url');
 return encoded+'.'+signature(encoded);
}
