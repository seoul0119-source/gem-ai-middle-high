...
import {validatedMembership,requiresMembership,membershipDeadline} from "./membership.js";

export function isBlockedStudentId(id) { return String(id || "").trim().toUpperCase() === "R260001"; }

// Classification is derived only from a sheet-validated ID, never from client input.
export function studentRegistration(id) {
  const normalized = String(id || '').trim().toUpperCase();
  if (!/^[A-Z][0-9]{6}$/.test(normalized)) return { registrationType: 'unknown', registrationLabel: '등록 확인 필요', isTrial: false };
  const isTrial = normalized.startsWith('T');
  return { registrationType: isTrial ? 'trial' : 'regular', registrationLabel: isTrial ? '체험 학생' : '정식 등록 학생', isTrial };
}
...
export function createSessionToken(student) {
  if (isBlockedStudentId(student?.id)) return null;
  const membership = validatedMembership(student.id, student.membership);
  if (requiresMembership(student.id) && !membership) return null;
  const now = Math.floor(Date.now() / 1000);
  if (membershipDeadline(membership) <= now) return null;
  ...
  const payload = {
    ...
    scienceRunHistory: Array.isArray(student.scienceRunHistory)
      ? student.scienceRunHistory.filter(id => typeof id === "string" && /^science-v2:[0-9a-f-]{36}$/i.test(id)).slice(-20)
      : [],
    startedAt: student.startedAt || null,
    endedAt: student.endedAt || null,
    authenticatedAt,
    iat: now,
    exp: Math.min(now + SESSION_MAX_AGE, absoluteExpiry, membershipDeadline(membership))
  };
...
export function readStudentSession(request) {
  ...
  try {
    ...
    const now = Math.floor(Date.now() / 1000);
    if (isBlockedStudentId(payload.id)) return null;
    ...
    const membership = validatedMembership(payload.id, payload.membership);
    if (requiresMembership(payload.id) && !membership) return null;
    if (membershipDeadline(membership) <= now || Number(payload.exp) > membershipDeadline(membership)) return null;
    if (membership) payload.membership = membership;
    Object.assign(payload, studentRegistration(payload.id));
