...
// Match the classroom's plain-text display before validating model output.
// Formatting alone must not hide an otherwise complete question from parsers.
// Never repair numbers, add a label, or manufacture a missing choice.
export function normalizeGeneralSuneungDisplay(value) {
  return String(value || "")
...
export function validateGeneralSuneungTurn({ text, record, intent, currentQuestion, start, completed, previousAttempts = 0 }) {
  ...
  if (record) {
    ...
    if (!headers.length) return "missing_next_question_header";
    if (headers.length > 1) return "multiple_next_question_headers";
    if (headers[0].question !== currentQuestion + 1) return "wrong_next_question_number";
    if (!isCompleteGeneralQuestion(text)) return "next_question_choices_incomplete";
    return "";
  }
