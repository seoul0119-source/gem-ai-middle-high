...
export async function verifyReviewRequest(body, keys=REVIEW_PUBLIC_KEYS) {
 try {
  ...
  if(p.purpose!=='gem-math-review-v1'||!keys[p.classroom]||!Number.isSafeInteger(p.timestamp)||Math.abs(Date.now()-p.timestamp)>60000)return null;
...
export function reviewModelRequest(p) {
 const language=p.classroom==='fr'?'French':'English';
