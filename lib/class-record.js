...
export function issueRecordPermit(student){return token({purpose:'record-run',student:student.id,course:student.courseId,id:student.courseRunId,started:student.startedAt,exp:Math.floor(Date.now()/1000)+7*86400});}
export function recordFromRequest(student,body){
 ...
 const messages=r.messages.map(m=>{if(!['user','assistant'].includes(m.role)||typeof m.content!=='string'||m.content.length>16000)throw Error('올바르지 않은 대화입니다.');return {role:m.role,content:m.content};});
 const counters={current:Math.max(1,Math.min(10,Number(r.counters?.current)||1)),uncertain:!!r.counters?.uncertain,items:{}};
 for(let i=1;i<=10;i++){const q=r.counters?.items?.[i];if(q)counters.items[i]={incorrect:Math.max(0,Math.min(400,Number(q.incorrect)||0)),hints:Math.max(0,Math.min(400,Number(q.hints)||0))};}
 const record={id:p.id,student:p.student,course:p.course,started:p.started,revision:r.revision,status:r.status,messages,counters};
